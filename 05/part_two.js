...
const reorder = (numbers) => {
  const ordered_number = [...numbers];
  ...
  for (let i = 0; i < numbers.length; i++) {
    ...
    numbers
      .filter((number) => number != nb)
      .forEach((other) => {
        const common_rules = rules.filter((rule) => rule.includes(nb) && rule.includes(other));

        for (let k = 0; k < common_rules.length; k++) {
          const nb_rule = common_rules[k];
          const parts = nb_rule.split("|");

          if (parts.indexOf(nb) == 1) postition++;
        }
      });

    ordered_number[postition] = nb;
  }
...
for (let i = 0; i < updates.length; i++) {
  ...
  const ordered_update = reorder(numbers);

  const center = ordered_update[Math.floor(numbers.length / 2)];
